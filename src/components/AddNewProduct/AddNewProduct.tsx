...
const AddNewProduct: FC = () => {
  ...
  const formik = useFormik({
    initialValues: { name: '', category: '', price: '', image: '' },
    validationSchema,
  ...
  return (
    <div className="add-form-page" dir="rtl">
      <form className="add-form-card" onSubmit={formik.handleSubmit}>
        ...
        <div className="add-form-card__field">
          <label>שם:</label>
          <input name="name" type="text" placeholder="שם המוצר" {...formik.getFieldProps('name')} />
          {formik.touched.name && formik.errors.name && <div className="add-form-card__error">{formik.errors.name}</div>}
        </div>

        <div className="add-form-card__field">
          <label>קטגוריה:</label>
          <input name="category" type="text" placeholder="קטגוריה" {...formik.getFieldProps('category')} />
          {formik.touched.category && formik.errors.category && <div className="add-form-card__error">{formik.errors.category}</div>}
        </div>

        <div className="add-form-card__field">
          <label>מחיר:</label>
          <input name="price" type="number" placeholder="מחיר" {...formik.getFieldProps('price')} />
